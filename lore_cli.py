...
def recent(
    limit: int = typer.Option(
        10, "--limit", "-l", help="Number of recent items to show"
    ),
):
    """
    ...
    for i, item in enumerate(recent_items[:limit], 1):
        ...
        table.add_row(
            str(i), item.get("title", "Unknown"), item.get("type", "unknown"), viewed_at
        )
...
def create(
    ...
    category: Optional[str] = typer.Option(
        None, "--category", "-c", help="Category (for entries)"
    ),
    description: Optional[str] = typer.Option(
        None, "--description", "-d", help="Description (for books)"
    ),
):
    """
...
def link(
    ...
    if success:
        console.print(
            f"[green]✓[/green] Linked [cyan]{entry['title']}[/cyan] to [cyan]{book['title']}[/cyan]"
        )
    else:
...
"""
    ...
    console.print(Panel(tools_text, title="🛠️  Tool Reference", border_style="cyan"))
...
"""
    ...
    console.print(Panel(info_text, title="ℹ️  Lore CLI Info", border_style="cyan"))
