...
generate_lore_entry() {
    ...
    mv "${TEMP_FILE}.json" "$SKOGAI_DIR/knowledge/expanded/lore/entries/$ENTRY_ID.json"
...
generate_persona() {
    ...
    # Extract traits and voice
    TRAITS=$(echo "$RESPONSE" | grep -i "TRAITS:" | head -n 1 | sed 's/TRAITS://i' | xargs)
    VOICE=$(echo "$RESPONSE" | grep -i "VOICE:" | head -n 1 | sed 's/VOICE://i' | xargs)
...
generate_lorebook() {
    ...
    # Create the lorebook
    $SKOGAI_DIR/tools/manage-lore.sh create-book "$title" "$description"
...
link_persona_with_lore() {
    ...
    BOOKS=($($SKOGAI_DIR/tools/manage-lore.sh list-books | grep -v "No lore books found" | grep -oE 'book_[0-9_]+' | head -n $book_count))
    
    if [ ${#BOOKS[@]} -lt $book_count ]; then
        # Generate a new book if we don't have enough
        PERSONA_NAME=$($SKOGAI_DIR/tools/create-persona.sh show "$persona_id" | grep "=== Persona:" | sed 's/=== Persona: //' | sed 's/ ===//')
        ...
        # Get the new book ID
        NEW_BOOK=$(ls -t $SKOGAI_DIR/knowledge/expanded/lore/books/ | head -n 1 | sed 's/\.json//')
        BOOKS+=($NEW_BOOK)
    fi
    
    # Link each book to the persona
    for book_id in "${BOOKS[@]}"; do
        $SKOGAI_DIR/tools/manage-lore.sh link-to-persona "$book_id" "$persona_id"
